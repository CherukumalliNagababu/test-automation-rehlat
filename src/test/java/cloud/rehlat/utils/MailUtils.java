...
public class MailUtils {
	...
	public String readMailAndGetRecoveryCode(String mailBody) {
		...
		// if(mailBody.contains("Enter") && mailBody.contains("to access Canopy
		// Visualizer.")) {
		if (mailBody.contains("Your OTP:")) {
			// String mailFirstLine =
			// mailBody.substring(mailBody.indexOf("Enter"),
			// mailBody.indexOf("Visualizer"));
			String mailFirstLine = mailBody.substring(mailBody.indexOf("Your OTP:"),
					mailBody.indexOf("Expires in: 15 minutes only"));
	...
	/**
	 * This method is used to fetch the email body based on content type is
	 * Multipart
	 * 
	 * @param m
	 * @return
	...
	/*
	 * This method returns the link which matches the content form the html
	 * string
	 */
	...
	/*
	 * @Desc: This function is used to connect to gmail and search the mail with
	 * subject in gmail account
	 */
	public static String getMail(String subject) throws Exception {
		...
		try {
			...
			// store.connect("dev.slando.com",993, "build",
			// "Ruo7aidu1maNgoovailahngai");
			// System.out.println(store);
			/*
			 * If the mail is found then the loop below terminates with the
			 * return statement If the mail is not in the in box then it pauses
			 * for few seconds and check again It repeats for 20 times before it
			 * confirms that there is not mail in the inbox
			 */
			for (int count = 0; count < 60;) {
				...
				System.out.println(messages.length);
				/*
				 * this for loop Starts with the latest mails in the inbox and
				 * goes on.
				 */
				for (int i = messages.length - 1; i >= 0; i--) {
					// System.out.print(messages[i].getSubject());
					// This if condition checks for the mail with the given
					// subject
